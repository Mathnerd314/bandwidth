...
// FileHandle2 has a .read(upToCount:) method. Just invoking read() will cause an ambiguity warning. Use _read instead.
...
internal func _NSErrorWithErrno(_ posixErrno : Int32, reading : Bool, path : String? = nil, url : URL? = nil, extraUserInfo : [String : Any]? = nil) -> NSError {
    ...
    if let path = path {
        userInfo[NSFilePathErrorKey] = unsafeBitCast(path, to: NSString.self)
    } else if let url = url {
...
/* On Darwin, FileHandle2 conforms to NSSecureCoding for use with NSXPCConnection and related facilities only. On swift-corelibs-foundation, it does not conform to that protocol since those facilities are unavailable. */

open class FileHandle2 : NSObject {
#if os(Windows)
    ...
    private func _checkFileHandle2() {
    ...
    private var queue: DispatchQueue {
        ...
        if let queue = _queue {
            ...
            let storage = DispatchQueue(label: "com.apple.NSFileHandle2.fd_monitoring")
            _queue = storage
            return storage
        }
    }
    ...
    private func monitor(forReading reading: Bool, resumed: Bool = true, handler: @escaping (FileHandle2, DispatchSourceProtocol) -> Void) -> DispatchSourceProtocol {
        _checkFileHandle2()
    ...
    private var _readabilityHandler: ((FileHandle2) -> Void)? = nil // Guarded by privateAsyncVariablesLock
    open var readabilityHandler: ((FileHandle2) -> Void)? {
        get {
            privateAsyncVariablesLock.lock()
        ...
        set {
            ...
            if let handler = newValue {
                // The handler can be called as part of the creation of the monitoring source, which can then call into FileHandle2 code again. Make sure we do not hold the lock when this is invoked.
    ...
    private var _writeabilityHandler: ((FileHandle2) -> Void)? = nil // Guarded by privateAsyncVariablesLock
    open var writeabilityHandler: ((FileHandle2) -> Void)? {
        get {
            privateAsyncVariablesLock.lock()
    ...
    internal func _readDataOfLength(_ length: Int, untilEOF: Bool, options: NSData.ReadingOptions = []) throws -> NSDataReadResult {
        ...
        if length == 0 && !untilEOF {
          ...
          return NSDataReadResult(bytes: nil, length: 0, deallocator: nil)
        }
        ...
        if GetFileType(self._handle) == FILE_TYPE_DISK {
          ...
          if options.contains(.alwaysMapped) {
            ...
            return NSDataReadResult(bytes: pData, length: Int(szMapSize)) { buffer, length in
        ...
        return NSDataReadResult(bytes: data, length: total) { buffer, length in
          free(buffer)
        }
#else
        if length == 0 && !untilEOF {
        ...
        if statbuf.st_mode & S_IFMT == S_IFREG {
            ...
            if options.contains(.alwaysMapped) {
                ...
                if data != UnsafeMutableRawPointer(bitPattern: -1) {
                    return NSDataReadResult(bytes: data!, length: mapSize) { buffer, length in
        ...
        return NSDataReadResult(bytes: bytePtr, length: total) { buffer, length in
            free(buffer)
        }
#endif
    }

    public func _readBytes(into buffer: UnsafeMutablePointer<UInt8>, length: Int) throws -> Int {
#if os(Windows)
        var BytesRead: DWORD = 0
    ...
    public func readToEnd() throws -> Data? {
        guard self != FileHandle2._nulldeviceFileHandle2 else { return nil }
    ...
    public func write<T: DataProtocol>(contentsOf data: T) throws {
        guard self != FileHandle2._nulldeviceFileHandle2 else { return }
    ...
    public func offset() throws -> UInt64 {
        guard self != FileHandle2._nulldeviceFileHandle2 else { return 0 }
...
extension FileHandle2 {
    ...
    internal static var _stdinFileHandle2: FileHandle2 = {
        return FileHandle2(fileDescriptor: STDIN_FILENO, closeOnDealloc: false)
    }()

    open class var standardInput: FileHandle2 {
        return _stdinFileHandle2
    }

    internal static var _stdoutFileHandle2: FileHandle2 = {
        return FileHandle2(fileDescriptor: STDOUT_FILENO, closeOnDealloc: false)
    }()

    open class var standardOutput: FileHandle2 {
        return _stdoutFileHandle2
    }

    internal static var _stderrFileHandle2: FileHandle2 = {
        return FileHandle2(fileDescriptor: STDERR_FILENO, closeOnDealloc: false)
    }()

    open class var standardError: FileHandle2 {
        return _stderrFileHandle2
    }

    internal static var _nulldeviceFileHandle2: FileHandle2 = {
        class NullDevice: FileHandle2 {
    ...
    open class var nullDevice: FileHandle2 {
        return _nulldeviceFileHandle2
    }
    ...
    public convenience init(forReadingFrom url: URL) throws {
        let fd = try FileHandle2._openFileDescriptorForURL(url, flags: O_RDONLY, reading: true)
        self.init(fileDescriptor: fd, closeOnDealloc: true)
    }

    public convenience init(forWritingTo url: URL) throws {
        let fd = try FileHandle2._openFileDescriptorForURL(url, flags: O_WRONLY, reading: false)
        self.init(fileDescriptor: fd, closeOnDealloc: true)
    }

    public convenience init(forUpdating url: URL) throws {
        let fd = try FileHandle2._openFileDescriptorForURL(url, flags: O_RDWR, reading: false)
        self.init(fileDescriptor: fd, closeOnDealloc: true)
    }
}

extension NSExceptionName {
    public static let FileHandle2OperationException = NSExceptionName(rawValue: "NSFileHandle2OperationException")
}

extension Notification.Name {
    public static let NSFileHandle2ReadToEndOfFileCompletion = Notification.Name(rawValue: "NSFileHandle2ReadToEndOfFileCompletionNotification")
    public static let NSFileHandle2ConnectionAccepted = Notification.Name(rawValue: "NSFileHandle2ConnectionAcceptedNotification")
    public static let NSFileHandle2DataAvailable = Notification.Name(rawValue: "NSFileHandle2DataAvailableNotification")
}

extension FileHandle2 {
    public static let readCompletionNotification = Notification.Name(rawValue: "NSFileHandle2ReadCompletionNotification")
}

public let NSFileHandle2NotificationDataItem: String = "NSFileHandle2NotificationDataItem"
public let NSFileHandle2NotificationFileHandle2Item: String = "NSFileHandle2NotificationFileHandle2Item"
...
extension FileHandle2 {
    ...
    open func readInBackgroundAndNotify(forModes modes: [RunLoop.Mode]?) {
        ...
        let operation = { (_ data: DispatchData, _ error: Int32) in
            ...
            if error == 0 {
                userInfo[NSFileHandle2NotificationDataItem] = Data(data)
            } else {
                userInfo[NSFileHandle2NotificationDataItem] = Data()
#if os(Windows)
                ...
                userInfo["NSFileHandle2Error"] = Int(translatedError)
#else
                userInfo["NSFileHandle2Error"] = Int(error)
#endif
            }

            DispatchQueue.main.async {
                NotificationQueue.default.enqueue(Notification(name: FileHandle2.readCompletionNotification, object: self, userInfo: userInfo), postingStyle: .asap, coalesceMask: .none, forModes: modes)
            }
        }
    ...
    open func readToEndOfFileInBackgroundAndNotify(forModes modes: [RunLoop.Mode]?) {
        ...
        queue.async {
            ...
            DispatchQueue.main.async {
                ...
                if let error = error {
                    userInfo["NSFileHandle2Error"] = error
                }

                NotificationQueue.default.enqueue(Notification(name: .NSFileHandle2ReadToEndOfFileCompletion, object: self, userInfo: userInfo), postingStyle: .asap, coalesceMask: .none, forModes: modes)
            }
        }
    }
    ...
    open func acceptConnectionInBackgroundAndNotify(forModes modes: [RunLoop.Mode]?) {
        ...
        let owner = monitor(forReading: true, resumed: false) { (handle, source) in
            var notification = Notification(name: .NSFileHandle2ConnectionAccepted, object: handle, userInfo: [:])
            let userInfo: [AnyHashable : Any]

            let acceptedFD = accept(handle.fileDescriptor, nil, nil)
            if acceptedFD >= 0 {
                userInfo = [NSFileHandle2NotificationFileHandle2Item: FileHandle2(fileDescriptor: acceptedFD)]
            } else {
                userInfo = ["NSFileHandle2Error": NSNumber(value: errno)]
            }
            ...
            DispatchQueue.main.async {
                ...
                NotificationQueue.default.enqueue(Notification(name: .NSFileHandle2ConnectionAccepted, object: handle, userInfo: [:]), postingStyle: .asap, coalesceMask: .none, forModes: modes)
            }
        }
    ...
    open func waitForDataInBackgroundAndNotify(forModes modes: [RunLoop.Mode]?) {
        let owner = monitor(forReading: true, resumed: false) { (handle, source) in
            ...
            DispatchQueue.main.async {
                ...
                NotificationQueue.default.enqueue(Notification(name: .NSFileHandle2DataAvailable, object: handle, userInfo: [:]), postingStyle: .asap, coalesceMask: .none, forModes: modes)
            }
        }
...
open class Pipe: NSObject {
    public let FileHandle2ForReading: FileHandle2
    public let FileHandle2ForWriting: FileHandle2
    ...
    public override init() {
        ...
        self.FileHandle2ForReading = FileHandle2(handle: hReadPipe!,
                                               closeOnDealloc: true)
        self.FileHandle2ForWriting = FileHandle2(handle: hWritePipe!,
                                               closeOnDealloc: true)
#else
        ...
        switch (ret, errno) {
        case (0, _):
            self.FileHandle2ForReading = FileHandle2(fileDescriptor: fds.pointee, closeOnDealloc: true)
            self.FileHandle2ForWriting = FileHandle2(fileDescriptor: fds.successor().pointee, closeOnDealloc: true)
        ...
        case (-1, EMFILE), (-1, ENFILE):
            ...
            self.FileHandle2ForReading = FileHandle2(fileDescriptor: -1, closeOnDealloc: false)
            self.FileHandle2ForWriting = FileHandle2(fileDescriptor: -1, closeOnDealloc: false)
